...
void solve(){
    ...
    for(int i = 0; i < rev.length(); i++){
        if(rev[i] == '0') {
            ceros++;
            if(i && rev[i-1] == '0') zeros++;
            if(i == 0 && rev[rev.length()-1] == '0') zeros++;
        }
    }
    double res1 = ceros / (double)rev.length();
    double res2 = zeros / (double)ceros;
    if(res1>res2)cout << "Girar\n";
    else if (res2>res1) cout << "No girar\n";
    else cout << "Da igual\n";
}
/*
3
