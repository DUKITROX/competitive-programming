...
double metric(double Q, double q, int d, int t, int k) {
    double s1 = q/Q;
    double s2 = (10.0 ) / ((t + 10.0));
    double s3 = 1.0 / (pow(2.0, 0.1 * d));
    double s4 = (Q/Qt) / max(1.0, (double)k); 
    return 0.2*s2 + 0.3*s3 + 0.1*s4;
}
...
void solve(State& state) { // O(FN.M.N)
    for (int t = state.t_i; t < T; t++){
        ...
        for (const auto& req : requests) {
            ...
            if (flows[flow_idx].q > 1e-9 && available_bw[uav_x][uav_y] > 1e-9) {
        ...
        for(int i = 0; i < FN; i++)
            if(flows[i].t_0 <= t && flows[i].q > 1e-9 && (flows[i].schedule.empty() || get<0>(flows[i].schedule.back()) != t))
                flows[i].schedule.push_back({t,flows[i].m1, flows[i].n1, 0.0});
    }
}
...
int main() {
    ...
    prefix_B.resize(M+1, vector<double>(N+1, 0.0));
    for (int i = 0; i < M; ++i)
        for (int j = 0; j < N; ++j)
            prefix_B[i + 1][j + 1] = s0.uavs[i][j].B + prefix_B[i][j + 1] + prefix_B[i + 1][j] - prefix_B[i][j];
