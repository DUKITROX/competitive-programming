...
const double BOOST_FACTOR = 1.00; 
const double BOOST_ADD = 0.00; 
...
void astar(Estado& inicial, vector<Flow>& flows, vector<vector<UAV>>& uavs) {
    ...
    while (!pq.empty()) {
        ...
        if (e.t == T) {
            ...
            for (int flujo = 0; flujo < e.flows.size(); ++flujo) {
                for (int i = e.flows[flujo].m1; i <= e.flows[flujo].m2; ++i) {
                    for (int j = e.flows[flujo].n1; j <= e.flows[flujo].n2; ++j) {
                        ...
                        hijo.flows[flujo].score += 0.4 * ratio + 0.2 * (10 / double((hijo.t + 10))) * ratio + 0.3 * ratio * (1 / pow(2, 0.1 * (manhattan_distance(hijo.flows[flujo].access_x, hijo.flows[flujo].access_y, hijo.uavs[i][j].x, hijo.uavs[i][j].y))));
                        ...
                        if (pq.size() < MAX_SIZE) {
                            auto it = upper_bound(pq.begin(), pq.end(), hijo); 
                            pq.insert(it, hijo); 
                        }
                        else {
                            auto it = upper_bound(pq.begin(), pq.end(), hijo);
            ...
            //calcHeuristicF(e, e.flows[flujo]);
            //boostear
            //e.f = e.f * BOOST_FACTOR + BOOST_ADD;
            if (pq.size() < MAX_SIZE) {
                auto it = upper_bound(pq.begin(), pq.end(), e);
                pq.insert(it, e);
            }
            else {
                auto it = upper_bound(pq.begin(), pq.end(), e);
                if (it != pq.end()) pq.insert(it, e);
            }
        }
    }
...
int main() {
    ...
    for (int i = 0; i < FN; ++i) {
        ...
        flows[i].distMin = (flows[i].access_x <= flows[i].m2 && flows[i].access_x >= flows[i].m1 ? 0 : min(abs(flows[i].access_x - flows[i].m1), abs(flows[i].access_x - flows[i].m2))) + (flows[i].access_y <= flows[i].n2 && flows[i].access_y >= flows[i].n1 ? 0 : min(abs(flows[i].access_y - flows[i].n1), abs(flows[i].access_y - flows[i].n2)));
        QT += qt; 
    }
