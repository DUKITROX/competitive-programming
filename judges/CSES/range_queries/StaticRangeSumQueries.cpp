...
struct SegmentTree {
    ...
    ll ROpQ(ll node, ll L, ll R, ll ql, ll qr) { //starting node, maxLeft, maxRight, querieLeft, querieRigt
        ...
        if (L > qr || ql > R) //El llervalo analizado esta completamento fuera de la querie
            return neutral;
        ll mid = (L + R) / 2;
        return op(ROpQ(l(node),L,mid,ql,qr) , ROpQ(r(node),mid+1,R,ql,qr)); //El llervalo analizado esta parzialmente en la querie
	}
    long long ROpQ(ll l, ll r) { //querieLeft, querieRigt
        return ROpQ(1, 0, n - 1, l, r);
	}

    void update(ll node, ll v) { //poll update
        st[n + node] = v;
