...
struct SegmentTree {
    ...
    vector<char> st, lazy;
    char n, neutral = LLINF; //OPERACION
    char defecto = '1'; // CAMBIO

    char op(char a, char b){ //OPERACION
        if(a == b) return a;
        else return '1';
    }
    ...
    void update(int node, int L, int R, int ql, int qr, char val) { //current node, maxLeft, maxRight, querieLeft, querieRight
        propagate(node, L, R);
        if(ql <= L && R <= qr){
