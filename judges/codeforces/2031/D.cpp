...
void solve(){
	int n; cin >> n; vii arr(n); for(int i = 0; i < n; i++) {cin >> arr[i].first; arr[i].second = i;}
	for(int i = 0; i < n; i++){
		ii m = arr[i];
		if(i != 0) m = max(m, st.query(0,i-1));
		int ind = 
	}
}
...
//https://www.xvideos.com/video.udeuucmfa9b/_6_pollas_no_te_bastan_briseida_hoy_vas_a_recibir_un_gran_regalo.
